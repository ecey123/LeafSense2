...
MODEL_DIR = Path(r"C:\Users\eceol\Desktop\leafsense 2\models_finetune")
RESULTS_DIR = Path(r"C:\Users\eceol\Desktop\leafsense 2\results_finetune") / DATASET_NAME

MODEL_DIR.mkdir(parents=True, exist_ok=True)
RESULTS_DIR.mkdir(parents=True, exist_ok=True)

BEST_MODEL_PATH = MODEL_DIR / f"{DATASET_NAME}_best_finetune.keras"
CLASS_NAMES_PATH = MODEL_DIR / f"{DATASET_NAME}_class_names_finetune.json"
...
INITIAL_EPOCHS = INITIAL_EPOCH_CONFIG[DATASET_NAME]
FINE_TUNE_EPOCHS = FINE_TUNE_EPOCH_CONFIG[DATASET_NAME]
...
# Son 30 layer açık kalacak, geri kalanı donuk
...
plt.title(f"{DATASET_NAME} Confusion Matrix (Fine-Tuning)")
plt.tight_layout()
...
plt.ylim(0, 1.05)
plt.ylabel("Score")
plt.title(f"{DATASET_NAME} Precision / Recall / F1 (Fine-Tuning)")
plt.legend()
plt.tight_layout()
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.title(f"{DATASET_NAME} Accuracy (Initial + Fine-Tuning)")
plt.legend()
plt.grid(True)
plt.tight_layout()
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title(f"{DATASET_NAME} Loss (Initial + Fine-Tuning)")
plt.legend()
plt.grid(True)
plt.tight_layout()
